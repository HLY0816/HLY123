...
def reconcile_countries_by_name(plot_countries,gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
	...
	for item in plot_countries:
		for j in gdp_countries.values():
			if plot_countries[item]==j["Country Name"]:
				for year in range(1960,2016):
					if j[str(year)]=="":     #如果哪一年GDP存在，就加入字典里
						continue
					else:
						dilt[item]=j
	for code in plot_countries:      #没有GDP记录的国家不计入
		if code not in dilt:
			Set.add(code)
	return Tup
...
def build_map_dict_by_name(gdpinfo,plot_countries, year):
	...
	with open(gdpinfo["gdpfile"],"rt") as csvfile:
		...
		for row in reader:
			for item in plot_countries:
				country=plot_countries[item]
				if country==row[gdpinfo["country_name"]] and row[year]!="":
					Dict[item]=math.log10(float(row[year])) #有记录的年份就加入字典
				elif country==row[gdpinfo["country_name"]] and row[year]=="":
					Set2.add(item)       #没有GDP记录的国家
				else:
					continue
	Set1=tup0[1]            #没有任何GDP记录的国家代码
	Set3=Set2-Set1          #没有特定年份记录的国家
	tup=(Dict,Set1,Set3)
	return tup
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
	...
	worldmap_chart.title = "世界各国GDP的分布图"
	worldmap_chart.add(year,build_map_dict_by_name(gdpinfo,plot_countries, year)[0])  #有GDP记录的国家分布图
	worldmap_chart.add("missing from world bank",build_map_dict_by_name(gdpinfo,plot_countries, year)[1])  #没有任何GDP记录的国家图
	worldmap_chart.add("no data at this year",build_map_dict_by_name(gdpinfo,plot_countries, year)[2])    #没有特定记录的国家图
	worldmap_chart.render_to_file(map_file)   #输出结果
...
def test_render_world_map(year):  #测试函数
	...
	render_world_map(gdpinfo, pygal_countries, year, "qqisp_gdp_world_name_1988.svg") #调用对应的函数
	print("文件已经生成")
